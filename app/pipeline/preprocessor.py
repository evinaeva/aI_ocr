"""
Phase 3 — Image preprocessing helpers.
Implements: load_image, maybe_upscale, scale_bbox, crop_zone_to_png.
"""
...
# Upscale thresholds per Phase 3 v2 spec
_MIN_W = 1024
_MIN_H = 768
...
def maybe_upscale(
    ...
    Preserves aspect ratio. Uses LANCZOS resampler only.
    new_w/new_h computed with round() (NOT ceil/floor).
    """
    w, h = img.size
...
def scale_bbox(
    ...
    actual_size MUST always be processed_size, never original_size.
    floor for x1/y1, ceil for x2/y2, then clamp.
    Returns [x1, y1, x2, y2].
    """
...
def crop_to_base64(img: "Image.Image", bbox_scaled: List[int]) -> str:
    ...
    return base64.b64encode(crop_zone_to_png(img, bbox_scaled)).decode("ascii")
