...
class TestNormalizeLevels(unittest.TestCase):
    ...
    def test_strict_drops_decoration_brackets(self):
        # `[ ... ]` and `< ... >` around plain text are decoration;
        # the OCR will never pick them up and the operator should
        # not see phantom differences in the table.
    ...
    def test_soft_strips_non_whitelisted_brackets(self):
        # Whitelist is bypassed ("BUY TOKENS" is not a placeholder
        # name) but the surrounding brackets are still stripped as
        # decoration — comparison shouldn't flag them.
...
class TestCompareLines(unittest.TestCase):
    ...
    def test_decoration_brackets_do_not_block_pass(self):
        ...
        # the bare text. Brackets are stripped by normalization, so
        # this is PASS.
